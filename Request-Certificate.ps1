    ...
    function Remove-ReqFromStore {
        ...
        foreach ($certreq in $($certstore.Certificates)) {
            if ($certreq.Subject -eq "CN=$CN") {
                Write-Output "Removing existing pending request for CN: $CN"
...
PROCESS {
    ...
    $subjectVars = [ordered]@{ 'CN' = 'cn'; 'Country' = 'c'; 'State' ='s'; 'City' = 'l'; 'Organisation' = 'o'; 'Department' = 'ou'; }
    $subjectArray = New-Object System.Collections.ArrayList

    foreach ($var in $subjectVars.GetEnumerator()) {
        if (((Get-Variable -Name $var.Name -ValueOnly) -ne '') -and ($null -ne (Get-Variable -Name $var.Name -ValueOnly))) {
            $paramValue = Get-Variable -Name $var.Name -ValueOnly
            $subjectArray.Add("$($var.Value)=$paramValue") | Out-Null
        }
    }

    $file = @"
[NewRequest]
Subject = "$($subjectArray -join ',')"
MachineKeySet = TRUE
KeyLength = $KeyLength
KeySpec=1
Exportable = TRUE
